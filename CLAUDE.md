...
5. **Phases hand off through durable artifacts, not conversation — so a session can be cleared between them.** Each phase writes a self-sufficient durable artifact (refine → ticket/AC; design → EDD; design-review → `## Design Review` on the EDD; plan → `## Implementation Plan` on the EDD; implement → git commits; review → the PR). A downstream skill's *entry* must reconstruct everything it needs from that artifact, never from the prior conversation — so the workflow supports running each phase in its own cleared context ("phase-scoped sessions") to keep context small and relevant. Two seams get an **advisory** `/clear`-and-resume prompt from the exiting skill (naming the artifact): planning→implementation (end of `/plan`) and implementation→review (end of `/implement`). These prompts are **never required** — a user who ignores them and runs the whole chain in one session gets identical behaviour; a skill cannot clear its own session, so the prompt is the mechanism. `/retro` is the deliberate exception: it runs in the final (review) session and reads durable artifacts for evidence from earlier, cleared phases. When adding a phase-boundary skill, mirror this: durable self-sufficient output, a fresh-read entry, and (at a seam) an advisory handoff prompt.
