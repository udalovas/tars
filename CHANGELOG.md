...
- Orchestration mode — run the existing `/implement`, `/test`, and `/review`
  ...
  - New `docs/ORCHESTRATION.md` onboarding guide (stream/slug conventions,
    canonical gate definition, auto-mode permission guidance, native
    `--worktree`/`--name` stream launch, an optional Stop-hook template that
    makes the gate deterministic, and an enable checklist).
